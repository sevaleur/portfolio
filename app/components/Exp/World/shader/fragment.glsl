...
float scene(vec3 p)
{
    vec3 p1 = rotate(p, vec3(1., 1., 1.), 1.3 + (u_time * 0.03));
    float scale = 200. + 10. * sin(6.); 
    float main = max(sdBox(p1, vec3(.125, .125, .125) - vec3(u_size)), (noise(p1 * scale)) / scale);
    float mouse_sphere = sdSphere(p - vec3(u_mouse, .0), 0.02 - u_size);
...
vec3 GetColorAmount(vec3 p)
{
    ...
    vec3 col = 0.5 + 0.5 * cos((PI * 2.) * (vec3(0.6, 0.4, 0.2) + amount * vec3(.6, .2, .5)));
    return col * amount;
}
...
void main()
{
    ...
    for(int i = 0; i<=40; i++)
    {
        ...
        color += 0.02 * GetColorAmount(ray_pos);
    }
